...
export const HierarchicalBlock = ({ data, ancestors, context, dragDisabled }) => {
  ...
  return (
    <div style={dragBlockStyles} className={focused ? `focus-${frame}` : null}>
      <Row wrap={false} style={{ fontSize: 16, marginBottom: 7 }} align='middle' justify='space-between'>
        <Row wrap={false} align='middle' style={{ boxShadow: editing?'inset 0px 0px 2px 1px #ffffff':null, backgroundColor: 'rgba(255,255,255,0.1)', borderRadius: 3, padding: 4, textAlign: 'start', flex: 1, minWidth: 130, maxWidth: 200, cursor: dragDisabled ? "not-allowed" : "grab",zIndex:101, marginRight:5, height: 32 }}>
      ...
      <animated.div style={nodeListStyle}>
        <NodeList ancestors={primitiveAncestors} uuids={data.children} context={context} dragDisabled={!data.editable || dragDisabled}/>
      </animated.div>
    </div>
  )
};