...
public class EnvironmentScript : MonoBehaviour
{
    ...
    public EvD.Environment.EnvironmentContext environmentModel = null;
    ...
    private void Start()
    {
        ...
        var reachSphereModel = new EvD.Environment.ReachSphere();

        // Default Pinch Points
        var pinchPointsModel = new List<EvD.Environment.PinchPoint>();
        foreach (var p in pinchPoints.pinchPoints)
        {
            pinchPointsModel.Add(new EvD.Environment.PinchPoint(name: p.name));
        }

        // Default Collision Meshes
        var collisionMeshesModel = new List<EvD.Environment.CollisionMesh>();
        foreach (var c in collisionMeshes.collisionMeshes)
        {
            collisionMeshesModel.Add(new EvD.Environment.CollisionMesh(name: c.name));
        }

        // Default Occupancy Zones
        var occupancyZonesModel = new List<EvD.Environment.OccupancyZone>();
        var humanZone = new EvD.Environment.OccupancyZone(occupancy_type: "human", name: "human", posZ: -0.5f);
        occupancyZonesModel.Add(humanZone);
        occupancyZones.HumanOccupancyZone.model = humanZone;

        var robotZone = new EvD.Environment.OccupancyZone(occupancy_type: "robot", name: "robot");
        ...
        var locationsModel = new List<EvD.Data.Location>();

        // Default Trajectory List
        var trajectoriesModel = new List<EvD.Data.Trajectory>();

        var machinesModel = new List<EvD.Data.Machine>();

        var thingsModel = new List<EvD.Data.Thing>();

        var waypointsModel = new List<EvD.Data.Waypoint>();

        // Construct model
        environmentModel = new EvD.Environment.EnvironmentContext(
    ...
    private void LocationAndWaypointCallback(Dictionary<string, string> entry)
    {
        if (entry["verb"] == "set")
        {
            var w = (EvD.Data.Waypoint)(EvD.Cache.Get(entry["uuid"], "waypoint"));
