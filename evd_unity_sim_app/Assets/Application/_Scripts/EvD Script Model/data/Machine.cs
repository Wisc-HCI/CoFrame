    ...
    namespace Data
    {
        ...
        public class Machine : Node
        {
            ...
            */
            ...
            public static new Machine FromDict(Dictionary<string,object> dct)
            {

                var inputRegions = new Dictionary<string,Region>();
                foreach (KeyValuePair<string,Dictionary<string,object>> entry in (Dictionary<string,Dictionary<string,object>>)dct["input_regions"])
                {
                    inputRegions.Add(entry.Key,(Region)UtilityFunctions.NodeParser(entry.Value));
                }

                var outputRegions = new Dictionary<string,Region>();
                foreach (KeyValuePair<string,Dictionary<string,object>> entry in (Dictionary<string,Dictionary<string,object>>)dct["output_regions"])
                {
                    outputRegions.Add(entry.Key,(Region)UtilityFunctions.NodeParser(entry.Value));
                }

                var recipe = MachineRecipe.FromDict((Dictionary<string,object>)dct["recipe"]);
            ...
            */
            ...
            public override void Set(Dictionary<string, object> dct)
            {
                if (dct.ContainsKey("input_regions"))
                {
                    ...
                    foreach (KeyValuePair<string,object> entry in (Dictionary<string,object>)dct["input_regions"])
                    {
                        regions.Add(entry.Key,(Region)UtilityFunctions.NodeParser((Dictionary<string,object>)entry.Value));
                    }
                ...
                {
                    Dictionary<string,Region> regions = new Dictionary<string, Region>();
                    foreach (KeyValuePair<string,object> entry in (Dictionary<string,object>)dct["output_regions"])
                    {
                        regions.Add(entry.Key,(Region)UtilityFunctions.NodeParser((Dictionary<string,object>)entry.Value));
                    }
        ...
        public class MachineRecipe : Node
        {
            ...
            */
            ...
            public static new MachineRecipe FromDict(Dictionary<string,object> dct)
            {
                return new MachineRecipe(
                    processTime: (float)dct["process_time"],
            ...
            */
            ...
            public int GetInputThingQuantity(string thingType)
            {
