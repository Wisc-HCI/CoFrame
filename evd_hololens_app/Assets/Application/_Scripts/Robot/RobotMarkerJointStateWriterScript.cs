...
public class RobotMarkerJointStateWriterScript : MonoBehaviour
{
    ...
    public void SetJointStates(double[] positions)
    {
        for (int i = 0; (i < positions.Length) && (i < joints.Count); i++)
        {
            joints[i].writer.Write((float)positions[i]); 
        }
    }

    public void SetJointState(string name, double position)
    {
        for (int i = 0; i < joints.Count; i++)
        {
            if (joints[i].name == name)
            {
                joints[i].writer.Write((float)position);
            }
        }
    }
