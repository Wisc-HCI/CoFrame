...
impl JsSolver {
    ...
    pub fn new(
        ...
        let inner_bounds: Option<Vec<(f64, f64)>> =
            temp_bounds.map(|bs| bs.iter().map(|b| (b.value, b.delta)).collect());
    ...
    pub fn reset(&mut self, state: &JsValue, weights: &JsValue) {
