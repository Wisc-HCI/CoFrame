...
class BladeFeederMachine(MachineTemplate):
    ...
    def __init__(self, uuid, prefix=None, rate=5, simulated=True):
        ...
        super(BladeFeederMachine,self).__init__(
            uuid, prefix, 
            init_fnt=self._init,
    ...
    def spin(self):
        ...
        while not rospy.is_shutdown():
            ...
            if self.current_status == 'running' and self.elapsedTime() >= PROCESS_TIME_BLADE_FEEDER:
    ...
    def _stop(self):
        ...
        elif self.elapsedTime < PROCESS_TIME_BLADE_FEEDER: # if we stop early
            self.current_status = 'error'
        else:
            self.current_status = 'idle'
        return True
...
if __name__ == "__main__":
    rospy.init_node('blade_feeder_machine')

    uuid = rospy.get_param('uuid',MACHINE_UUID_BLADE_FEEDER)
    prefix = rospy.get_param('prefix',None)
    rate = rospy.get_param('rate',5)
    simulated = rospy.get_param('simulated',True)

    node = BladeFeederMachine(uuid,prefix,rate,simulated)
    node.spin()