...
export const computedSliceSubscribe = (useStore) => {
    useStore.subscribe(state =>
        [state.programData, state.focus, state.occupancyVisible, state.collisionsVisible, state.robotPreviewVisible, state.issues],
        (current, previous) => {
            // ===================== TFs =====================
            ...
            if (deepestIssue && deepestIssue.code === 'pinchPoints' && state.programData[deepestIssue.focus[0]]) {
                const pinchPointAnimations = pinchpointAnimationFromExecutable(Object.values(state.programData).filter(v => v.type === "robotAgentType")[0], compiledState[deepestIssue.focus[0]]?.[ROOT_PATH]?.steps)
