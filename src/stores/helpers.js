...
export function idleTimeEstimate(state, programSteps) {
    ...
    programSteps.forEach(step => {
        ...
        if(step.type === STEP_TYPE.ACTION_START && ["delayType", "processWaitType"].includes(state[step.source].type)) {
            open[step.source] = step.time;

        // Remove event from the open object and update the duration
        } else if(step.type === STEP_TYPE.ACTION_END && ["delayType", "processWaitType"].includes(state[step.source].type)) {
