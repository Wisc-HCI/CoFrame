...
export function getPlotInfo({focusItem}) {
    const colors = focusItem.graphData.lineColors;
    const showLegend = Object.keys(focusItem.graphData.series[0]).length > 2;
    const intervalLength = focusItem.graphData.series.length > 5 ? Math.floor(focusItem.graphData.series.length / 5) : 0;
    return (
        <div style={{ width: '100%', minHeight:'200px', height: '100%', fill: 'white'}}>
            {focusItem.graphData.title !== '' && <h3 style={{textAlign: 'center'}}>{focusItem.graphData.title}</h3>}
